...
5. **transcript 兜底判定**（未入账检测，双重保险）——**调用 `scripts/check_unread.py <项目目录>` 单次完成**（2026-08-10 提速：原判定需现写 python 且易按过时描述写错，改为 skill 自带脚本一次输出判定结果+尾部快览）：
   - 脚本逻辑：读 `.memory/state.json` 的 `saved_at`（保存流程写入的保存时刻）→ `saved_at` 缺失（旧项目）→ 输出 `UNREAD=1`（保守触发）→ 存在 → **单步判定**：最新存档（`.memory/transcripts/` 按 mtime 取最新一份）中是否存在 `saved_at` **之后产生的实质 user 消息**（「保存/退出」命令已被保存流程的结束时校准排除在基准前）→ 输出 `UNREAD=0/1` + 有则尾部 30 条快览
   - **判定要点（双格式兼容）**：Claude Code transcript 存在两种格式——
   ...
   - `UNREAD=0` → **残留同步**（保存流程尾部响应/汇报的 Stop 同步会推新存档 mtime——校准后 mtime 恒晚于 saved_at，故不做 mtime 比较，只认实质消息）→ **静默跳过**，不读尾部
   - `UNREAD=1` → **触发**
   - **触发** → 用脚本输出的尾部 30 条快览入摘要；未入账量大（>30 条）→ **弹 AskUserQuestion 卡片**：完整读 / 读尾部 / 跳过（成本用户决策）
...
3. **回顾本次对话**，将以下内容追加或更新到 CLAUDE.md：
   ...
   - ⚠️ **写入侧归属核查（硬防线）**：待办 / 下一步行动 / 已知问题写入前**逐条验证归属**——条目须能从本项目上下文（CLAUDE.md 现有内容 / 本次对话中归属于本项目的部分）解释；无法解释的条目 = 疑似跨项目串扰（跨项目会话的 transcript 存档混有其他项目对话，保存回顾时可能误抄入本项目）→ **禁止写入**，单独列出询问用户归属。实证：本项目曾混入量化项目待办（OpenExec 验证 / Windows 密码）长期未察觉（2026-08-09 用户追问才发现）
