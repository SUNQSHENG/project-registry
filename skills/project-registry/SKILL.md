...
5. **transcript 兜底判定**（未入账检测，双重保险）：
   ...
   - ⚠️ **串扰分辨（先行）**：存档为整个会话（跨项目时会含其他项目对话）——读取时**分辨内容归属**，只提取当前项目相关内容并入摘要，其他项目内容忽略；**若 saved_at 后实质 user 消息均为其他项目内容（无当前项目归属）→ 视为无未入账，静默跳过（不弹卡、不入摘要）**——判定阶段无法自动切分消息归属（已定案），靠触发后分辨兜底；跨项目误触发成本=一次毫秒级尾部读取
