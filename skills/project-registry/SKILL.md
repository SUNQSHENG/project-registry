...
第一次进入 skill 时，**逐项检查三项配置**（每项都用 AskUserQuestion 卡片询问，跳过即记录、只问一次）：
...
> 层 2 自动摘要是**增量草稿**（对话提炼合并进 CLAUDE.md），**不等于、也不能代替手动保存**。**手动保存 = 在对话中直接告诉 agent「保存」或「退出」**（没有按钮、没有命令，就是这么一句话）——触发时强制全面回顾、补录决策、按优先级更新下一步行动，才是**CLAUDE.md 全面更新**。配了 API 的用户同样**必须**在保存/退出时手动整理，不可因自动摘要而跳过。
...
- **未配置** → 卡片询问：「是否配置 API Key 启用自动摘要？」——**必须说明授权范围与功能价值**：
  ...
  >
  > ✅ **带来的功能**：
  > · CLAUDE.md 自动摘要——对话自动提炼进展/决策/待办/下一步写入（约 10 分钟级）
  > · 打开项目永远是最近状态
  > · 任意 OpenAI 兼容 API 可用（DeepSeek / OpenAI / 通义 / 本地 Ollama）
  >
  > ⚠️ **但自动摘要不能代替手动保存**：自动摘要 ≠ CLAUDE.md 全面更新；「保存/退出」时的全面回顾 + 下一步行动更新仍必须手动执行

  - **配置（推荐）** → 引导设置三个环境变量（README 有各提供商示例）——配置后同样必须按保存/退出流程手动整理
  - **跳过** → 写入 `<skill 目录>/config.json`；CLAUDE.md 内容靠手动保存，数据安全（层 1）不受影响
  - **了解更多** → 展示功能对比表

**功能对比：**

| 能力 | 零配置 | 层 1（hooks） | 层 1 + 层 2（hooks + Key） |
|:---|:---:|:---:|:---:|
...
2. `cd` 到项目目录
3. **触发「🧭 会话开始流程」**（回顾上次进展）
4. **声明当前项目**：「📌 当前项目：`<key>`，自动摘要/会话归档写入此项目」——会话级显式绑定，操作聚焦本项目
...
> **原理**：层 1 / 层 2 三个 hooks 脚本（transcript-sync.py / auto-summary.py / session-end.py）全部基于 `os.getcwd()` 定位当前项目（`is_project_dir()` 逻辑）——cwd 在 `<项目根>/<key>/`（默认 `~/projects/`）下才操作该项目，否则**跳过**。cwd 漂移会导致：自动摘要/transcript 同步写到错误项目，或全部跳过；会话结束归档（备份+git 提交）漏掉当前项目。
...
自动摘要 = **增量草稿**（增量合并）；手动保存 = **CLAUDE.md 全面更新**（全面回顾）——两者互补，**不可互相代替**：自动摘要不能跳过手动保存，手动保存也不受自动摘要影响。最终记录以手动保存为准。

> **手动保存怎么操作？** 在对话中对 agent 说「保存」或「退出」即可——没有独立按钮或命令。agent 会执行 CLAUDE.md 全面更新（全面回顾 + 补录决策及原因 + 按优先级重排下一步行动）。**随时可以说**，不依赖层 1/层 2 是否启用。
