...
> 层 2 自动摘要只是「实时保鲜」（增量合并），**不等于、也不能代替手动保存**。**手动保存 = 在对话中直接告诉 agent「保存」或「退出」**（没有按钮、没有命令，就是这么一句话）——触发时强制全面回顾、补录决策、按优先级更新下一步行动，才是**CLAUDE.md 全面更新**。配了 API 的用户同样**必须**在保存/退出时手动整理，不可因自动摘要而跳过。
...
- **未配置** → 卡片询问：「是否配置 API Key 启用自动摘要？」——**必须说明授权范围与功能价值**：
  ...
  >
  > ⚠️ **但自动摘要不能代替手动保存**：保鲜 ≠ CLAUDE.md 全面更新；「保存/退出」时的全面回顾 + 下一步行动更新仍必须手动执行
...
自动摘要 = **实时保鲜**（增量合并）；手动保存 = **CLAUDE.md 全面更新**（全面回顾）——两者互补，**不可互相代替**：自动摘要不能跳过手动保存，手动保存也不受自动摘要影响。最终记录以手动保存为准。
