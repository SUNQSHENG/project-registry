...
**自动保鲜 ≠ 手动保存。** 层 2 自动摘要是*增量*保鲜——实时草稿，让 CLAUDE.md 在两次保存之间保持不过期。**它不能代替手动保存**：保存/退出时的全面回顾，才补录决策（含 WHY）、按优先级重排下一步行动。自动保鲜让记录"保鲜"，手动保存让记录"正确"——权威版本永远是你手动保存的那份。
...
| 项目 CLAUDE.md 自动保鲜 | ✅（可选，任意 API） | ❌（写全局记忆目录） |
