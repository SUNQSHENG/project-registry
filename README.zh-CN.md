...
保存项目会记录保存时刻（saved_at，保存流程结束时校准）；打开项目时会检查对话存档（上下文自动备份写入的 `.memory/transcripts/` 对话原文）中是否有保存时刻**之后产生的实质对话**——有则会话回顾读取对话原文尾部，**未保存的对话永不静默丢失**（量大时询问是否完整读取；跨项目会话自动分辨归属，其他项目的对话不会误触发本项目的检查）。零配置即用：上下文自动备份数据安全 + 全部核心功能。
